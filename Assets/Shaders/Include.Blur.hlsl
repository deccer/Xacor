...
float4 Blur_GaussianBilateral(float2 uv, Texture2D sourceTexture, Texture2D depthTexture, Texture2D normalTexture, SamplerState bilinearSampler, float2 texelSize, float2 direction, float sigma)
{
	...
	float3 center_normal = NormalDecode(normalTexture.SampleLevel(bilinearSampler, uv, 0).xyz);
	...
	for (int i = -5; i < 5; i++)
	{
		...
		float3 sample_normal = NormalDecode(normalTexture.SampleLevel(bilinearSampler, texCoord, 0).xyz);
